...
const UPDATE_CHECK_ALARM = "shanna-auth-update-check";
const UPDATE_RELEASE_URL = "https://github.com/shannacore/shannauth/releases/latest";
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  checkForUpdate().then(sendResponse).catch(() => sendResponse(null));
  return true;
});
...
async function checkForUpdate() {
  ...
  const response = await fetch(
    "https://api.github.com/repos/shannacore/shannauth/releases/latest",
  ...
  if (!/^\d+(\.\d+){0,3}$/.test(latestVersion)) {
    return { available: false, currentVersion };
  }

  const toNumbers = (version: string) =>
    version.split(".").map((part) => Number(part) || 0);
  const current = toNumbers(currentVersion);
  const latest = toNumbers(latestVersion);
  const available = [0, 1, 2, 3].some((index) => {
  ...
  await chrome.storage.local.set({ shannaAuthUpdate: result });
  return result;
}
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (isChrome) {
    url = chrome.runtime.getURL("view/chrome-issues.html");
  }
